  ...
  Channel = (function() {
    ...
    function Channel(name, socket) {
      ...
      this.socket.on('message', function(msg) {
        var args, channel, event, _ref;
        try {
          _ref = JSON.parse(msg), channel = _ref.channel, event = _ref.event, args = _ref.args;
          if (channel !== _this.name) {
            return;
          }
          if (!Array.isArray(args)) {
            args = [args];
          }
          return _this.realEmit.apply(_this, [event].concat(__slice.call(args)));
        } catch (e) {
          throw e;
        }
      });
...
(function() {
  ...
  Pulsar = (function(_super) {
    ...
    Pulsar.prototype.connected = false;

    Pulsar.prototype.channels = {};
